/**
 * Vercel Serverless Function: /api/concerts
 * Fetches upcoming Bay Area Chinese concerts from Ticketmaster Discovery API v2
 *
 * Requirements:
...
function mapEventToConcert(event: any): ConcertItem {
...
async function fetchTMSearch(apiKey: string, keyword: string): Promise<any[]> {
  ...
  // Ticketmaster wants ISO format without milliseconds: 2025-03-03T00:00:00Z
  ...
  try {
    ...
    if (!response.ok) {
      console.warn(`[Concerts] Ticketmaster "${keyword}" returned ${response.status}`);
      return [];
    }
    ...
    clearTimeout(timeoutId);
    console.error(`[Concerts] Failed to fetch "${keyword}":`, error instanceof Error ? error.message : error);
    return [];
  }
}

async function fetchConcerts(nocache: boolean = false): Promise<{ items: ConcertItem[]; sourceMode: 'live' | 'cache' | 'unavailable' }> {
  const cacheKey = 'concerts_bayarea';

  if (!nocache) {
    const cached = getCachedData(cacheKey, CONCERTS_CACHE_TTL, false);
    if (cached?.data?.items) {
      console.log(`[Concerts] ✅ Using cache (${cached.data.items.length} items)`);
  ...
  if (!apiKey) {
    console.warn('[Concerts] TICKETMASTER_API_KEY not set');
    const stale = getStaleCache(cacheKey);
    const staleItems: ConcertItem[] | undefined = stale?.data?.items;
  ...
  for (const event of [...results1, ...results2, ...results3]) {
    ...
    const concert = mapEventToConcert(event);
    if (concert.url) {
      merged.push(concert);
    }
  }
  ...
  if (items.length > 0) {
    ...
    console.log(`[Concerts] ✅ Fetched ${items.length} concerts`);
  ...
  const staleItems: ConcertItem[] | undefined = stale?.data?.items;
  if (staleItems && staleItems.length > 0) {
    console.log(`[Concerts] ✅ Using stale cache (${staleItems.length} items)`);
    return { items: staleItems, sourceMode: 'cache' };
  }

  console.log('[Concerts] No concerts found');
  return { items: [], sourceMode: 'unavailable' };
}
...
export async function handleConcerts(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const { items, sourceMode } = await fetchConcerts(nocache);
    ...
    res.status(200).json({
      ...
      count: items.length,
      fetchedAt,
      ttlSeconds: ttlMsToSeconds(CONCERTS_CACHE_TTL),
      sourceMode,
    });
  } catch (error) {
    console.error('[API /api/concerts] Error:', error);

    const stale = getStaleCache('concerts_bayarea');
    const staleItems: ConcertItem[] | undefined = stale?.data?.items;
    if (staleItems && staleItems.length > 0) {
