      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-4">
              ...
              {marketNews.slice(0, 3).map((item: any, index: number) => (
                ...
                >
                  <div className="flex items-start gap-1">
                    ...
                    <span className="text-[14px] font-normal font-mono text-foreground/90 group-hover:text-primary transition-colors line-clamp-2 flex-1">
        ...
        <div className="min-w-0 md:border-l md:border-border/10 md:pl-4">
          <div className="mb-2 flex items-center justify-between">
            ...
            <a
              href="https://www.1point3acres.com/bbs/forum.php?mod=forumdisplay&fid=291&filter=author&orderby=dateline"
              target="_blank"
              rel="noopener noreferrer"
              className="text-xs opacity-60 hover:opacity-100 transition-opacity font-mono font-normal"
            >
            ...
            <div className="space-y-0.5" style={{ lineHeight: '1.4' }}>
              {leekItems.map((item, index) => (
                <a
                  ...
                  rel="noopener noreferrer"
                  className={`block py-1 hover:bg-card/80 transition-all group rounded-sm ${
                    index < leekItems.length - 1 ? 'border-b border-border/30' : ''
                  }`}
                >
                  <div className="flex items-start gap-1">
                    <span className="text-primary mt-0.5 text-[10px] flex-shrink-0">•</span>
                    <span className="text-[14px] font-normal font-mono text-foreground/80 group-hover:text-primary transition-colors line-clamp-2 flex-1">
                      {item.title}
                    </span>
                  </div>
                </a>
