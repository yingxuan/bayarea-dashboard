...
const FALLBACK_HOUSING_VIDEO: HousingVideo = {
  title: "本周湾区华人房市视频",
...
export default function Fangzi() {
  ...
  const [housingVideo, setHousingVideo] = useState<HousingVideo>(FALLBACK_HOUSING_VIDEO);
  ...
  useEffect(() => {
    ...
    const loadHousingVideo = async () => {
      try {
        ...
        const latest = result?.videos?.[0];
        if (!latest || cancelled) return;

        setHousingVideo({
          title: latest.title || FALLBACK_HOUSING_VIDEO.title,
          subtitle: latest.channelTitle
            ? `${latest.channelTitle} · ${new Date(latest.publishedAt).toLocaleDateString("en-US")}`
            : FALLBACK_HOUSING_VIDEO.subtitle,
          url: latest.url || FALLBACK_HOUSING_VIDEO.url,
          image: latest.thumbnail || FALLBACK_HOUSING_VIDEO.image,
        });
      } catch {
        // keep fallback card
      }
        ...
        <div className="route-shell mx-auto w-full max-w-6xl px-4 py-4 md:px-6 md:py-6">
          ...
          <section className="section-shell rounded-sm p-5">
            <div className="grid gap-4 xl:grid-cols-[0.95fr_1.05fr]">
              ...
              <a
                href={housingVideo.url}
                target="_blank"
                rel="noopener noreferrer"
                onClick={handleExternalLinkClick}
                className="group block overflow-hidden rounded-sm border border-border/25 bg-background/35 transition-all hover:border-primary/35 hover:bg-background/55"
              >
                <div className="relative aspect-video overflow-hidden bg-muted">
                  <img
                    src={housingVideo.image}
                    alt={housingVideo.title}
                ...
                <div className="p-4">
                  <div className="text-lg font-semibold text-foreground">{housingVideo.title}</div>
                  <div className="mt-2 text-sm text-muted-foreground">{housingVideo.subtitle}</div>
                </div>
              </a>
            </div>
          </section>
                  ...
                  <div className="rounded-sm border border-border/25 bg-background/35 p-4">
                    ...
                    <Carousel opts={{ align: "start", loop: false, dragFree: true }} className="w-full">
                      <CarouselContent className="-ml-3">
                        {(openHousesByZip[market.zip] || market.openHouses).map((home) => (
                          <CarouselItem
